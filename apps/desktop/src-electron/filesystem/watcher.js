...
class Watcher {
  ...
  watch(win, watchPath, type = "dir" /* file or dir */) {
    ...
    const usePolling = isOsx
      ? true
      : this._preferences.getItem("watcherUsePolling");
    ...
    watcher
      .on("add", async (pathname) => {
        if (
          !(await this._shouldIgnoreEvent(win.id, pathname, type, usePolling))
        ) {
          const { _preferences } = this;
          const eol = _preferences.getPreferredEol();
          const { autoGuessEncoding, trimTrailingNewline } =
            _preferences.getAll();
  ...
  async _shouldIgnoreEvent(winId, pathname, type, usePolling) {
    if (type === "file") {
      ...
      for (let i = 0; i < _ignoreChangeEvents.length; ++i) {
        const {
          windowId,
          pathname: pathToIgnore,
          start,
          duration,
        } = _ignoreChangeEvents[i];
        if (windowId === winId && pathToIgnore === pathname) {
          ...
          if (!usePolling) {
            try {
              ...
              console.error(
                'Failed to "stat" file to determine modification time:',
                error
              );
            }
          }
