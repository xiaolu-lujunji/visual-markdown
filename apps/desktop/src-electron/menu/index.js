...
class AppMenu {
  ...
  getRecentlyUsedDocuments() {
    ...
    try {
      const recentDocuments = JSON.parse(
        fs.readFileSync(RECENTS_PATH, "utf-8")
  ...
  addEditorMenu(window, options = {}) {
    ...
    const sourceCodeModeMenuItem = menu.getMenuItemById(
      "sourceCodeModeMenuItem"
    );
    sourceCodeModeMenuItem.checked = isSourceMode;

    if (isSourceMode) {
      const typewriterModeMenuItem = menu.getMenuItemById(
  ...
  _listenForIpcMain() {
    ...
    ipcMain.on("mt::update-format-menu", (e, windowId, formats) => {
      if (!this.has(windowId)) {
        log.error(
          `UpdateApplicationMenu: Cannot find window menu for window id ${windowId}.`
        );
        return;
      }
