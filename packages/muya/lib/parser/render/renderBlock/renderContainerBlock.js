  ...
  if (/^(?:th|td)$/.test(type)) {
    ...
    if (cells && cells.length) {
      ...
      if (findTable && renderingRowContainer) {
        ...
        if (
          block.parent === activeBlocks[1].parent &&
          !block.preSibling &&
          tableRow > 0
        ) {
    ...
    if (
      /html|multiplemath|flowchart|mermaid|sequence|plantuml|vega-lite/.test(
        functionType
      )
    ) {
    ...
    if (
      /html|multiplemath|mermaid|flowchart|vega-lite|sequence|plantuml/.test(
        functionType
      )
    ) {
